...
class PriorityQueueTest {
	...
	static class PQMySort implements Comparator<Integer> {  
		//based on even or odd
		public int compare(Integer one, Integer two) {
			if (one %2 == 0) {
				return two % 2 == 0 ? 1:0;
			}
	...
	public static void main(String[] args) {
		...
		PriorityQueue<Integer> pqq = new PriorityQueue<Integer>(10, new PQMySort()); // use
		// Comparator
		
		
		//PriorityQueue<Integer> pqq = new PriorityQueue<Integer>();
		for (int i = 0; i < ia.length; i++) {
			pqq.offer(ia[i]); //9856173  via my priority queu 
		}
		...
		//Normal QUEUE is first-in-first-out
		//But this queue is based on Comparator you use 
		//if not used comparator, then Natural order (Comparable), if not overridden compareTo then will fail
...
//			System.out.print(pq2.peek());
		...
		System.out.println();
		PriorityQueue<Integer> pq3 = new PriorityQueue<Integer>(10, pqs); // use
		...
		PriorityQueue<String> pq4 = new PriorityQueue<String>();
		for (String s : sa)
			pq4.offer(s);
		for (String s : sa)
