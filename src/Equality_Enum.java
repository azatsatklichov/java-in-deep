...
class EnumEqual {
	 ...
	 * Equality for Enums 
	 * 
	 * Once you've declared an enum, it's not expandable. At
	 * runtime, there's no way to make additional enum constants. Of course, you
	 * can have as many variables as you'd like refer to a given enum constant,
	 * so it's important to be able to compare two enum reference variables to
	 * see if they're "equal", i.e. do they refer to the same enum constant. You
	 * can use either the == operator or the equals() method to determine if two
	 * variables are referring to the same enum constant:
	 * 
	 * @param args
	 */
