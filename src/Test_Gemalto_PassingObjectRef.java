 ...
 * 
 */
public class Test_Gemalto_PassingObjectRef {

	public static void main(String[] args) {
		Dimension d = new Dimension(5, 10);
		Test_Gemalto_PassingObjectRef rt = new Test_Gemalto_PassingObjectRef();
	 ...
	 * ReturnTypeOverload objects have been created, one referenced by the local variable f and
	 * one referenced by the local (argument) variable g. Because the doStuff()
	 * method has a copy of the reference variable, it has a way to get to the
	 * original ReturnTypeOverload object, for instance to call the setName() method. But, the
	 * doStuff() method does not have a way to get to the f reference variable.
	 * So doStuff() can change values within the object f refers to, but
	 * doStuff() can't change the actual contents (bit pattern) of f. In other
	 * words, doStuff() can change the state of the object that f refers to, but
	 * it can't make f refer to a different object!
	 * 
	 * @param g
	 */
